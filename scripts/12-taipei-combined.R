### ============================================================
### Combined Taipei Map + Chronological Figure
### ============================================================
### Four species: G. triflora, G. heterocarpa, M. fauriei, M. pumilus
###
...
###
### Usage:
###   source("scripts/12-taipei-combined.R")
### ============================================================
...
species_shapes <- c(
    ...
    "M. fauriei"     = 0,   # square
    "M. pumilus"     = 2    # triangle
)

### ============================================================
### 2. Load specimen data (all 4 species)
### ============================================================
...
mazus_pumilus     <- read_csv("data/processed/tai_mazus_pumilus_taiwan.csv",      show_col_types = FALSE)
mazus_fauriei     <- read_csv("data/processed/tai_mazus_fauriei_taiwan.csv",     show_col_types = FALSE)

all_specimens <- bind_rows(
    grona_triflora    |> mutate(taxon = "G. triflora"),
    grona_heterocarpa |> mutate(taxon = "G. heterocarpa"),
    mazus_fauriei     |> mutate(taxon = "M. fauriei"),
    mazus_pumilus     |> mutate(taxon = "M. pumilus")
) |>
...
message("All 4 species — Greater Taipei: ", nrow(taipei_specimens))
message("By species:")
...
### ============================================================
### 6. Panel D: Specimen histogram (all 4 species)
### ============================================================
bin_width <- 5
...
species_dots <- taipei_specimens |>
    ...
    mutate(x_offset = case_when(
        taxon == "G. triflora"    ~ -1.8,
        taxon == "G. heterocarpa" ~ -0.6,
        taxon == "M. fauriei"     ~  0.6,
        taxon == "M. pumilus"     ~  1.8
    ))
...
zoom_shade <- data.frame(
    x = c(0.6, 0.73, 0.73, 0.6),
    y = c(0.95, 0.72, 0.65, 0.61)
)
...
p_left <- ggdraw() +
    ...
    draw_plot(
        ...
        x = ins_x, y = ins_y, width = ins_w, height = ins_h
    ) +
...
ggsave("plots/12-taipei-combined.png", p_out,
       width = 7, height = 4, dpi = 800)

message("Saved: plots/12-taipei-combined.png")
